...
export default function EditPropertyPage() {
  ...
  useEffect(() => {
    ...
    if (property) {
      setForm({
        ...
        lat: String(property.location.coordinates.lat),
        lng: String(property.location.coordinates.lng),
  ...
  const updateNearbyPlace = (i: number, field: string, val: string) => {
    ...
    (places[i] as Record<string, string>)[field] = val;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setSaving(true);
    console.log("Updated property:", { id: params.id, ...form });
    alert("Property updated! (API integration pending)");
    setSaving(false);
    ...
    <div>
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          ...
          <p className="text-[#888] mt-1">{form.title || "Loading..."}</p>
        </div>
      </div>
          ...
          {form.features.map((feature, i) => (
            <div key={i} className="flex items-center gap-2 mb-2">
              ...
              {form.features.length > 1 && (
                <button type="button" onClick={() => removeFeature(i)} className="text-red-400 hover:text-red-300 text-xs">
                  Remove
                </button>
              )}
            </div>
